...
async function openModelExplorer() {
  ...
  function renderBody() {
    ...
    if (active === "Overview") {
      ...
      const rows = Object.entries(params)
        ...
        .map(([n, d]) => detailRow(n, d));
      content = rows.length ? el("div", {}, ...rows) : emptyMsg();
  ...
  nav.replaceChildren(
    ...
    el("button", { class: "tiny", title: "Raw JSON", onclick: () =>
      openModal(el("h3", {}, "Model JSON"),
        el("pre", { class: "log" }, JSON.stringify(raw, null, 2).slice(0, 400000)),
        el("div", { class: "row gap", style: "margin-top:10px;justify-content:flex-end" },
          el("button", { onclick: openModelExplorer }, "Back"))) },
      "{ } raw"),
  );
